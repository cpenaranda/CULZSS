...
__global__ void DecodeKernelSmash(unsigned char * in_d, unsigned char * out_d,
  ...
  int index_in = 0;
  uint32_t index_out = bx * chunk_size;
  int size_packet = 0;
  for (uint32_t i_bx = 0; i_bx < bx; ++i_bx) {
    index_in += compressed_size[i_bx];
  }
  size_packet = compressed_size[bx];

  if (size_packet >= chunk_size) {
    // Compressed data was bigger than uncompressed data, so we copied the
    // uncompressed data.
    uncompressed_size[bx] = chunk_size;
    while(chunk_size--) {
      out_d[index_out++] = in_d[index_in++];
    }
    return;
  } else if (size_packet == 0) {
    // Compressed data was bigger than uncompressed data, so we copied the
    // uncompressed data.
    uncompressed_size[bx] = compressed_size[bx + 1];
    chunk_size = compressed_size[bx + 1];
    while(chunk_size--) {
      out_d[index_out++] = in_d[index_in++];
    }
    return;
  ...
  *****************************************************************************/
  ...
  while (TRUE) {
    ...
    if (flagsUsed == 8) {
      // shifted out all the flag bits, read a new flag
      if (filepoint >= size_packet) {
        break;
      }
      c = in_d[index_in + filepoint]; // packet * PCKTSIZE
      ++filepoint;
      flags = c & 0xFF;
      flagsUsed = 0;
    }
    ...
    if (flags & 0x01) {
      ...
      // write out byte and put it in sliding window
      out_d[index_out + wfilepoint] = in_d[index_in + filepoint];
      ++wfilepoint;
      slidingWindow[nextChar] = in_d[index_in + filepoint];
      nextChar = (nextChar + 1) % WINDOW_SIZE;
      ++filepoint;
    } else {
      // offset and length
      ...
      code.length = in_d[index_in + filepoint];
      ++filepoint;

      if (filepoint >= size_packet) {
        break;
      }
      code.offset = in_d[index_in + filepoint];
      ++filepoint;
	...
	uncompressed_size[bx] = wfilepoint;
}

void DecodeKernelSmash(const uint32_t &batch, const cudaStream_t &stream,
                  char *device_compressed_data, char *device_decompressed_data,
                  uint32_t * device_compressed_sizes,
                  uint32_t * device_uncompressed_sizes,
                  const uint32_t &chunk_size) {
  DecodeKernelSmash<<<batch, 1, 0, stream>>>(
      reinterpret_cast<unsigned char*>(device_compressed_data),
      reinterpret_cast<unsigned char*>(device_decompressed_data),
      device_compressed_sizes, device_uncompressed_sizes, chunk_size);
}
