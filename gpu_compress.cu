...
__global__ void EncodeKernelSmash(char * in_d, uint64_t in_size, char * out_d,
                                  uint32_t chunk_size) {
  /* cyclic buffer sliding window of already read characters */
  __shared__ unsigned char slidingWindow[WINDOW_SIZE+(MAX_CODED)];
  __shared__ unsigned char uncodedLookahead[MAX_CODED*2];
  ...
  *****************************************************************************/
  ...
  // in_d[bx * chunk_size + tx];
  // uncodedLookahead[tx] = tex1Dfetch(in_d_tex, bx * chunk_size + tx);
  uint32_t index = bx * chunk_size + tx;
  uint64_t max_size =
      (in_size < ((bx+1)*chunk_size + tx)) ? in_size :
                                             (bx+1)*chunk_size + tx;
  if (index >= max_size) return;
  uncodedLookahead[tx] = in_d[index];
  filepoint += MAX_CODED;

  slidingWindow[ (windowHead + WINDOW_SIZE ) % (WINDOW_SIZE + MAX_CODED) ] =
      uncodedLookahead[uncodedHead];
  // uncodedLookahead[uncodedHead];
  // tex1Dfetch(in_d_tex, bx * chunk_size + tx);

  __syncthreads();

  // in_d[bx * chunk_size + filepoint + tx];
  // uncodedLookahead[MAX_CODED+tx] =
  //     tex1Dfetch(in_d_tex, bx * chunk_size + filepoint + tx);
  index += filepoint;
  if (index >= max_size) return;
  uncodedLookahead[MAX_CODED + tx] = in_d[index];
  filepoint+=MAX_CODED;
  ...
  loadcounter++;
  // Look for matching string in sliding window
  matchData = FindMatch(windowHead, uncodedHead, slidingWindow,
                        uncodedLookahead, tx, bx, 0, 0, loadcounter);
  __syncthreads();

  // now encoded the rest of the file until an EOF is read
  while ((filepoint) <= chunk_size && !lastcheck) {
    if (matchData.length >= MAX_CODED) {
      // garbage beyond last data happened to extend match length
      matchData.length = MAX_CODED-1;
    }
    ...
    // write out the encoded data into output
    out_d[bx * chunk_size*2 + wfilepoint + tx*2] = encodedData[tx*2];
    out_d[bx * chunk_size*2 + wfilepoint + tx*2 + 1] = encodedData[tx*2 + 1];
    // update written pointer and heads
    wfilepoint += (MAX_CODED*2);
    ...
    // if (!lastcheck)
    {
      if (filepoint < chunk_size) {
        // uncodedLookahead[(uncodedHead+ MAX_CODED)% (MAX_CODED*2)] =
        //     tex1Dfetch(in_d_tex, bx * chunk_size + filepoint + tx);
        index = bx * chunk_size + filepoint + tx;
        if (index >= max_size) return;
        uncodedLookahead[(uncodedHead + MAX_CODED) % (MAX_CODED * 2)] =
            in_d[index];
        filepoint+=MAX_CODED;
        ...
        // find the location for the thread specific view of window
        slidingWindow[ (windowHead + WINDOW_SIZE ) %
                       (WINDOW_SIZE + MAX_CODED) ] =
            uncodedLookahead[uncodedHead];
        // __syncthreads();
      } else{
        lastcheck++;
        slidingWindow[(windowHead + MAX_CODED ) %
                      (WINDOW_SIZE + MAX_CODED)] = '^';
      }
      __syncthreads();
      ...
      loadcounter++;
      matchData =
          FindMatch(windowHead, uncodedHead, slidingWindow,
                    uncodedLookahead, tx, bx, wfilepoint, lastcheck,
                    loadcounter);
    }
  } // while
...
__global__ void AfterCompressionKernel(char * uncompressed_data_d, char * in_d,
                                       char * out_d, uint32_t uncompressed_size,
                                       uint32_t chunk_size, uint32_t *sizes) {
  int bx = blockIdx.x;
  int tx = threadIdx.x;
  bool bad_compression = false;
  int index_in  = 0;
  int initial_index_out = 0;
  int index_out = 0;
  int finish = 0;
  int mark = 0;
  ...
  uint32_t slices = uncompressed_size / chunk_size;
  uint32_t u_size =
      (slices == bx) ? (uncompressed_size % chunk_size) : chunk_size;
  ...
  index_in = bx * chunk_size * 2;
  index_out = bx * chunk_size * 2;
  initial_index_out = index_out;
  finish = (bx + 1) * chunk_size * 2;

  while (!bad_compression && index_in < finish) {
    mark = in_d[index_in];
    // if no matching
    if (mark == 1) {
      // mark with uncoded byte flag
      flags |= flagPos;
      holdbuf[holdbufcount] = in_d[index_in + 1];
      ++holdbufcount;
      index_in += 2;
    } else {
      // if there is matching
      holdbuf[holdbufcount] = mark;
      ++holdbufcount;
      holdbuf[holdbufcount] = in_d[index_in + 1];
      ++holdbufcount;
      index_in += (mark*2);
    ...
    if (flagPos == 0x80) {
      // if we have looked at 8 characters that fills the flag holder
      out_d[index_out] = flags;
      ++index_out;

      while (i_hold < holdbufcount) {
        out_d[index_out++] = holdbuf[i_hold++];
      }
    ...
    bad_compression = ((index_out-initial_index_out) >= u_size);
  }
  if (!bad_compression) {
    if (holdbufcount > 0) {
      out_d[index_out] = flags;
  ...
  if (bad_compression) {
    int index_uncompressed = bx * chunk_size;
    index_out = bx * chunk_size * 2;
    if (slices == bx) {
      sizes[bx] = 0;
      sizes[bx + 1] = u_size;
    ...
    while (u_size--) {
      out_d[index_out++] = uncompressed_data_d[index_uncompressed++];
    }
  } else {
    sizes[bx] = index_out - (bx * chunk_size * 2);
  }
}
...
void EncodeKernelSmash(const uint32_t &batch, const cudaStream_t &stream,
  ...
  EncodeKernelSmash<<<batch, MAX_CODED, 0, stream>>>(
    device_uncompressed_data, uncompressed_size, device_compressed_data,
    chunk_size);
  AfterCompressionKernel<<<batch, 1, 0, stream>>>(
    device_uncompressed_data, device_compressed_data, device_compressed_data,
    uncompressed_size, chunk_size, device_compressed_sizes);
}
